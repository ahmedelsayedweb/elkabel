...
class LocationsPlugin extends GoldPlugin
{
	...
	function __construct()
	{
		...
		if ( !$this->isValidKey() ) {
			$cb_settings = array(
				...
				'pitch' => 'Upgrade now to instantly unlock the Store Locator widget, and help your customers find the nearest store to them.',
				'learn_more_url' => 'https://goldplugins.com/downloads/locations-pro/',
				'upgrade_url' => 'https://goldplugins.com/special-offers/upgrade-to-locations-pro/',
				'text_domain' => 'wordpress',
				'testimonial' => false,
			);
			$this->coupon_box = new GP_MegaSeptember($cb_settings);
		}
		...
		if( is_admin() && ( empty($_REQUEST['post_type']) || $_REQUEST['post_type'] !== 'location' ) && ($cur_post_type !== 'location') )
		{
			...
			$Locations_MediaButton->add_button('Single Location', 'locations', 'singlelocationwidget', 'location-alt');
			$Locations_MediaButton->add_button('List of Locations',  'locations', 'locationslistwidget', 'location-alt');
			
			if( $this->isValidKey() ){
				$Locations_MediaButton->add_button('Store Locator',  'store_locator', 'storelocatorwidgetajax', 'location-alt');
				$Locations_MediaButton->add_button('Search Locations',  'store_locator', 'storelocatorwidget', 'location-alt');
			}
		}
		...
		register_activation_hook( __FILE__, array($this, 'flush_location_rewrite_rules') );	
	...
	function add_hooks()
	{
		...
		add_action( 'wp_ajax_locations_pro_ajax_search', array($this,'handle_ajax_searches') );
		add_action( 'wp_ajax_nopriv_locations_pro_ajax_search', array($this,'handle_ajax_searches') );		
	...
	function locations_add_admin_css($hook){
		...
		if ( strpos($hook,'locations')!==false || $screen->id === "widgets" || is_customize_preview() ){
			...
			wp_enqueue_script(
				'gp-admin_v2',
				plugins_url('assets/js/locations_sajak.js', __FILE__),
				array( 'jquery' ),
				false,
				true
			);	
	...
	function handle_ajax_searches()
	{
		...
		$paged = filter_input( INPUT_POST, 'paged', FILTER_SANITIZE_STRING );
	...
	{		
		...
		if ( in_array($atts['search_box_location'], array('above', 'top', 'both')) ) {
			...
			$current_search = isset( $_REQUEST['your_location'] ) ? $_REQUEST['your_location'] : '';
		...
		{			
			switch( $search_results_style ) {
				case 'tile':
					...
					$location_html = $this->build_location_html($loc_obj, $atts);
	...
	function store_locator_item_html($loc, $link_title = false)
	{
		...
		$html .= sprintf('<li id="%s" class="location noPhoto">', $location_id);
			...
			if ( !empty($directions_link) ) {
				$html .= sprintf('<div class="directions-wrapper">%s</div>', $directions_link);
			}
	...
	private function setup_tabs()
	{
		...
		$this->tabs->add_tab(
			'import_export', 
			'Import / Export',
			array($this, 'output_import_export_fields'),
			array(
				'icon' => 'map-marker',
	...
	function get_pro_options($load_values = true)
	{
		$pro_options = array();
		...
		$pro_options[] = array('name' => 'loc_p_search_radius', 'label' => 'Search Radius', 'desc' => 'When a user searches for nearby locations, show stores that are within this distance.', 'class' => 'small' );
		$pro_options[] = array('name' => 'loc_p_default_latitude', 'label' => 'Default Latitude:', 'desc' => 'If we are unable to geolocate the visitor by their IP address, center the map to this latitude.' , 'default' => '39.8282', 'class' => 'small'); // default to USA
		$pro_options[] = array('name' => 'loc_p_default_longitude', 'label' => 'Default Longitude:', 'desc' => 'If we are unable to geolocate the visitor by their IP address, center the map to this longitude.', 'default' => '-98.5795', 'class' => 'small'); // default to USA
...
?>
		...
		<div class="wrap locations-admin-wrap <?php echo $this->isValidKey() ? 'gp_pro' : 'gp_not_pro';?>">
		<?php
			// output Sajak tabs
			$this->tabs->display(); 
			if ( !$this->isValidKey() ) {
				echo $this->coupon_box->form();
			}
		?>	
		</div>		
...
<?php
	...
	?>
		...
		<?php else: ?>
		<p><em>Locations Pro is required to use the Store Locator function.</em></p>
		<?php endif; ?>
	<?php	
	}
	
	function output_import_export_fields()
	{
		$pro_options = $this->get_pro_options();
	?>
		<h3>Import / Export Your Locations</h3>
		<div class="gp_csv">
...
?>
		<tr valign="top">
			...
			<?php
					...
					default:
			?>
					<input type="text" id="<?php echo $opt['name'] ?>" name="<?php echo $opt['name'] ?>" size="25" value="<?php echo $val ?>" class="<?php echo $class ?>" />
			<?php
					break;
...
?>
		...
		<div id="signup_wrapper">
			<div class="topper">
				<h3>Save 10% on Locations Pro!</h3>
				<p class="pitch">Sign-up for our newsletter, and we’ll send you a coupon for 10% off your upgrade to Locations Pro!</p>
			</div>
			<div id="mc_embed_signup">
				...
				<form action="https://goldplugins.com/atm/atm.php?u=403e206455845b3b4bd0c08dc&amp;id=6ad78db648" method="post" id="mc-embedded-subscribe-form" name="mc-embedded-subscribe-form" class="validate" target="_blank" novalidate>
					...
					<input type="hidden" id="mc-upgrade-link-per" value="https://goldplugins.com/purchase/locations-pro/single?promo=newsub10" />
					<input type="hidden" id="mc-upgrade-link-biz" value="https://goldplugins.com/purchase/locations-pro/business?promo=newsub10" />
					<input type="hidden" id="mc-upgrade-link-dev" value="https://goldplugins.com/purchase/locations-pro/developer?promo=newsub10" />
					<div class="features">
...
<?php
	...
	function process_export()
	{
		...
		if (isset($_REQUEST['_locs_pro_do_export']) && $_REQUEST['_locs_pro_do_export'] == '_locs_pro_do_export') {
	...
	function output_locations_column_content($column, $location_id)
	{
		 switch ( $column )
		 {
			...
			case 'latitude' :
				...
				echo get_post_meta( $location_id , $meta_key , true ); 
				break;
			...
			case 'mailing_address' :
				...
				echo htmlentities( get_the_title($location_id) ) . '<br>';
				echo $this->build_address_html(
