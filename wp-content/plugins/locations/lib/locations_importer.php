...
class LocationsPlugin_Importer
{
	...
	private function import_locations_from_csv($locations_file){	
		...
		foreach($locations as $location){		
			...
			if(empty($postslist)){				
				...
				// assign Categories if any were specified
				...
				if (!empty($location['Categories'])) {
					$post_cats = explode(',', $location['Categories']);
					$post_cats = array_map('intval', $post_cats); // sanitize to ints
					wp_set_object_terms($new_id, $post_cats, 'location-categories');
				}
	...
	public function csv_importer(){
		...
		$current_url .= '#tab-import_export';
		//printf( '<div class="gp_upload_form" data-gp-ajax-form="1" method="POST" action="%s" enctype="multipart/form-data">', $current_url );
		...
		if(empty($_FILES)){		
			...
			echo "<p><strong>Expected Headers:</strong></p>";
			echo "<p><code>Name,Street Address,Street Address (line 2),City,State,Zipcode,Phone,Website,Email,Fax,Latitude,Longitude,Categories</code></p>";
...
//					'action' => $current_url
				...
				ob_start();					
				wp_import_upload_form( add_query_arg($action_params) );
				$import_form_html = ob_get_contents();
				$current_url = add_query_arg( array() );
				$current_url .= '#tab-import_export';				
				$replacement = sprintf( '<div data-gp-ajax-form="1" action="%s" ', $current_url );
				$import_form_html = str_replace('<form ', $replacement, $import_form_html);
